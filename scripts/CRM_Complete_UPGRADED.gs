...
function doGet(e) {
  try {
    ...
    if (action === 'get_jeftinije_feed') {
      var pricing = getLatestPricing();
      var xml = buildJeftinijeXml(pricing.prices);
...
function sendCustomOffer(isMobile) {
  ...
  if (qrBlob && qrDataUri) {
    ...
    inlineImages["qrcode"] = qrBlob; // Key is CID
...
function testZohoFullSend() {
  console.log("--- POKREĆEM FULL SEND TEST (v3.3) ---");
  ...
  // Test HTML s CID-om
  var html = "<h1>Zoho Test v3.3</h1><p>Barkod ispod ovog teksta:</p><p><img src='cid:qrcode'></p><p>PDF u privitku.</p>";
...
// ── JEFTINIJE.HR XML FEED INTEGRATION ──
const BASE_URL_JEFTINIJE = 'https://2lmf-pro.hr';
...
function buildJeftinijeXml(prices) {
  const L = ['<?xml version="1.0" encoding="UTF-8"?>', '<CNJExport>'];

  for (const p of JEFTINIJE_PRODUCTS) {
    const price = prices[p.sku];
    if (!price || price <= 0) continue;

    const imgUrl     = p.img.startsWith('http') ? p.img : `${BASE_URL_JEFTINIJE}/${p.img}`;
    const productUrl = `${BASE_URL_JEFTINIJE}/proizvod.html?sku=${p.sku}`;
